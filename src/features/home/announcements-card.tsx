...
export function AnnouncementsCard({ announcements, mobileOnly = false, desktopOnly = false }: { announcements: Announcement[]; mobileOnly?: boolean; desktopOnly?: boolean }) {
  ...
  return (
    <aside className={`relative right-auto top-auto z-10 mx-auto mb-8 block w-[calc(100%-1rem)] max-w-[360px] rounded-2xl border border-primary/40 bg-background/80 p-3 shadow-glow-md backdrop-blur-md lg:absolute lg:right-[4%] lg:top-10 lg:mx-0 lg:mb-0 lg:block lg:w-[360px] lg:p-5 ${mobileOnly ? "lg:hidden" : ""} ${desktopOnly ? "hidden lg:block" : ""}`} aria-label="Live updates">
