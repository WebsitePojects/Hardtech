...
export function AnnouncementsCard({ announcements, mobileOnly = false, desktopOnly = false }: { announcements: Announcement[]; mobileOnly?: boolean; desktopOnly?: boolean }) {
  ...
  return (
    // lg:top-[75px] clears the fixed navbar, which is 70px tall at rest — the
    // reference leaves a 5px gap there. Anything smaller tucks the card under
    // the bar, where it covers the account chip. z-30 matches the reference and
    // keeps the card above the hero glow but below the navbar itself.
    <aside className={`relative right-auto top-auto z-30 mx-auto mb-8 block w-[calc(100%-1rem)] max-w-[360px] rounded-2xl border border-primary/40 bg-background/80 p-3 shadow-glow-md backdrop-blur-md lg:absolute lg:right-6 lg:top-[75px] lg:mx-0 lg:mb-0 lg:block lg:w-[360px] lg:p-5 ${mobileOnly ? "lg:hidden" : ""} ${desktopOnly ? "hidden lg:block" : ""}`} aria-label="Live updates">
